...
function budgetMiddleware(provider) {
...
app.post('/api/dataforseo/ranked-keywords', expensiveLimiter, budgetMiddleware('dataforseo'), async (req, res) => {
    ...
    try {
        const data = await withCache(realtimeCache, `dfs_rkw_${target}_${limit}`, async () => {
            const r = await axios.post('https://api.dataforseo.com/v3/dataforseo_labs/google/ranked_keywords/live', [{ target, language_name: 'English', location_code: 2840, limit: limit || 50 }], { headers: { Authorization: `Basic ${DATAFORSEO_AUTH}`, 'Content-Type': 'application/json' } });
            return r.data;
        });
        res.json(data);
    }
...
app.post('/api/dataforseo/competitors', expensiveLimiter, budgetMiddleware('dataforseo'), async (req, res) => {
    ...
    try {
        const data = await withCache(historicalCache, `dfs_comp_${target}_${limit}`, async () => {
            const r = await axios.post('https://api.dataforseo.com/v3/dataforseo_labs/google/competitors_domain/live', [{ target, language_name: 'English', location_code: 2840, limit: limit || 10 }], { headers: { Authorization: `Basic ${DATAFORSEO_AUTH}`, 'Content-Type': 'application/json' } });
            return r.data;
        });
        res.json(data);
    }
...
app.post('/api/thorbit/analyze', expensiveLimiter, budgetMiddleware('thorbit'), async (req, res) => {
    ...
    try {
        const data = await withCache(historicalCache, `thorbit_${url}_${keyword}`, async () => {
            if (!THORBIT_API_KEY)
                throw new Error('Thorbit API key not configured');
...
app.get('/api/overview', expensiveLimiter, async (req, res) => {
    const { domain } = req.query;
    const today = new Date().toISOString().split('T')[0];
    const result = { domain, sources: {}, activeSources: [] };
    const calls = await Promise.allSettled([
        ...
        axios.post('https://api.dataforseo.com/v3/backlinks/domain_pages_summary/live', [{ target: domain, include_subdomains: true }], { headers: { Authorization: `Basic ${DATAFORSEO_AUTH}`, 'Content-Type': 'application/json' }, timeout: 10000 }),
        // SE Ranking overview
    ...
    if (ahrefsDR.status === 'fulfilled') {
        ...
        if (ahrefsMetrics.status === 'fulfilled')
            result.sources.ahrefs.metrics = ahrefsMetrics.value.data;
        result.activeSources.push('Ahrefs');
    }
    if (semrush.status === 'fulfilled') {
        const rows = parseSemrushCSV(semrush.value.data);
        if (rows[0]) {
            result.sources.semrush = rows[0];
            result.activeSources.push('SEMrush');
        }
    }
...
app.get('/api/keywords/aggregated', expensiveLimiter, async (req, res) => {
    ...
    const calls = await Promise.allSettled([
        ...
        axios.post('https://api.dataforseo.com/v3/dataforseo_labs/google/ranked_keywords/live', [{ target: domain, language_name: 'English', location_code: 2840, limit: Number(limit) || 50 }], { headers: { Authorization: `Basic ${DATAFORSEO_AUTH}`, 'Content-Type': 'application/json' }, timeout: 10000 }),
    ...
    if (ahrefs.status === 'fulfilled') {
        result.sources.ahrefs = ahrefs.value.data;
        result.activeSources.push('Ahrefs');
    }
    if (semrush.status === 'fulfilled') {
        result.sources.semrush = parseSemrushCSV(semrush.value.data);
        result.activeSources.push('SEMrush');
    }
    if (dataforseo.status === 'fulfilled') {
        result.sources.dataforseo = dataforseo.value.data;
        result.activeSources.push('DataForSEO');
    }
    if (seranking.status === 'fulfilled') {
        result.sources.seranking = seranking.value.data;
...
app.get('/api/backlinks/aggregated', expensiveLimiter, async (req, res) => {
    ...
    if (ahrefsStats.status === 'fulfilled') {
        ...
        if (ahrefsRD.status === 'fulfilled')
            result.sources.ahrefs.refdomains = ahrefsRD.value.data;
        result.activeSources.push('Ahrefs');
    }
    if (dataforseo.status === 'fulfilled') {
        result.sources.dataforseo = dataforseo.value.data?.tasks?.[0]?.result?.[0];
...
app.post('/api/vitals/aggregated', expensiveLimiter, async (req, res) => {
    const { url } = req.body;
    const result = { url, sources: {}, activeSources: [] };
    const calls = await Promise.allSettled([
    ...
    if (psiMobile.status === 'fulfilled') {
        ...
        if (psiDesktop.status === 'fulfilled')
            result.sources.pagespeed.desktop = psiDesktop.value.data;
...
app.get('/api/competitors/aggregated', expensiveLimiter, async (req, res) => {
    ...
    const calls = await Promise.allSettled([
        ...
        axios.post('https://api.dataforseo.com/v3/dataforseo_labs/google/competitors_domain/live', [{ target: domain, language_name: 'English', location_code: 2840, limit: 10 }], { headers: { Authorization: `Basic ${DATAFORSEO_AUTH}`, 'Content-Type': 'application/json' }, timeout: 10000 }),
    ...
    if (exa.status === 'fulfilled') {
        result.sources.exa = exa.value.data?.results;
        result.activeSources.push('Exa');
    }
    res.json(result);
});
...
app.post('/api/content/analyze', expensiveLimiter, async (req, res) => {
    ...
    const result = { domain, keyword, sources: {}, activeSources: [] };
    const calls = await Promise.allSettled([
    ...
    if (exaOwn.status === 'fulfilled') {
        result.sources.exa = { ownContent: exaOwn.value.data?.results };
        result.activeSources.push('Exa');
    }
    if (exaCompetitive.status === 'fulfilled') {
        if (!result.sources.exa)
            result.sources.exa = {};
        result.sources.exa.competitiveContent = exaCompetitive.value.data?.results;
    }
    if (thorbit.status === 'fulfilled') {
        result.sources.thorbit = thorbit.value.data;
        result.activeSources.push('Thorbit');
    }
    res.json(result);
});
...
app.get('/api/alerts/aggregated', expensiveLimiter, async (req, res) => {
    ...
    if (ahrefsMetrics.status === 'fulfilled') {
        ...
        if (m?.org_traffic === 0)
            alerts.push({ severity: 'warning', source: 'Ahrefs', module: 'Traffic', message: 'No organic traffic detected', timestamp: new Date().toISOString() });
        if (m?.org_keywords === 0)
            alerts.push({ severity: 'info', source: 'Ahrefs', module: 'Keywords', message: 'No organic keywords ranking', timestamp: new Date().toISOString() });
    }
    if (pagespeed.status === 'fulfilled') {
        ...
        if (perf !== undefined && perf < 0.5)
            alerts.push({ severity: 'critical', source: 'PageSpeed', module: 'Vitals', message: `Performance score is ${Math.round(perf * 100)}/100 — needs improvement`, timestamp: new Date().toISOString() });
        else if (perf !== undefined && perf < 0.9)
            alerts.push({ severity: 'warning', source: 'PageSpeed', module: 'Vitals', message: `Performance score is ${Math.round(perf * 100)}/100`, timestamp: new Date().toISOString() });
    }
    if (ahrefsMetrics.status === 'rejected')
