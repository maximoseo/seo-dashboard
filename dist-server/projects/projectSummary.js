...
function stableProjectScore(project) {
...
function stableCount(project, seed, max) {
    const hash = [...`${project.domain}:${seed}`].reduce((sum, char) => sum + char.charCodeAt(0), 0);
    return hash % max;
}
export function buildProjectSummaries(seedProjects, source) {
    const fetchedAt = new Date().toISOString();
    const dataState = source === 'supabase' ? 'live' : 'cached';
...
export function getProjectByDomain(projects, domain) {
    if (!domain)
        return null;
    const normalized = domain.toLowerCase();
    return projects.find(project => project.domain.toLowerCase() === normalized) ?? null;
}
export function summarizeProjectModules(project) {
    return {
