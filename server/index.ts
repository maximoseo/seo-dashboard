      ...
      for (let i = 0; i < 6; i++) {
        ...
        if (status === 20000) {
          const pagesRes = await axios.get(
            `https://api.dataforseo.com/v3/on_page/pages/${taskId}`,
            { headers: { Authorization: `Basic ${DATAFORSEO_AUTH}` } }
          )
  ...
  try {
    const data = await withCache(realtimeCache, key, async () => {
      ...
      for (let i = 0; i < 12; i++) {
        await new Promise(r => setTimeout(r, 5000))
        const statusRes = await axios.get(
          `https://gtmetrix.com/api/2.0/tests/${testId}`,
  ...
  try {
    const data = await withCache(historicalCache, key, async () => {
      ...
      const r = await axios.post(
        ...
        params,
        { headers: { Authorization: `Bearer ${KEYWORDS_EVERYWHERE_API}`, 'Content-Type': 'application/x-www-form-urlencoded' } }
      )
      return r.data
    })
    res.json(data)
...
app.get('/api/overview', async (req, res) => {
  const { domain } = req.query as Record<string, string>
  const results: Record<string, any> = { domain, sources: {} }
  ...
  await Promise.allSettled([
    ...
    axios.post(
      ...
      { headers: { Authorization: `Basic ${DATAFORSEO_AUTH}`, 'Content-Type': 'application/json' } }
    ).then(r => { results.sources.dataforseo = r.data?.tasks?.[0]?.result?.[0] }),
  ])
...
app.get('/api/health', async (_req, res) => {
  const statuses: Record<string, { ok: boolean; latency?: number; error?: string }> = {}
