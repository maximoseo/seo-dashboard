...
export interface SeoSnapshotForAlerts {
  domain: string
  organicTraffic?: number | null
  previousOrganicTraffic?: number | null
  top10Keywords?: number | null
  previousTop10Keywords?: number | null
  brokenPagesWithBacklinks?: number | null
  performanceScore?: number | null
  providerErrors?: Array<{ provider: string; errorClass: string }>
}
...
function idFor(domain: string, module: string, title: string): string {
  return `${domain}:${module}:${title}`.toLowerCase().replace(/[^a-z0-9]+/g, '-')
}

export function generateAlerts(snapshot: SeoSnapshotForAlerts): SeoAlert[] {
